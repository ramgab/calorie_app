...
public class DinnerDatabaseHelper extends SQLiteOpenHelper {
    ...
    private static final String DATABASE_NAME = "dinner_database";
    private static final int DATABASE_VERSION = 2;

    static final String TABLE_DINNER = "dinner";
    ...
    // SQL query to create the dinner table
    private static final String CREATE_DINNER_TABLE = "CREATE TABLE " + TABLE_DINNER + " (" +
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        Log.d("DinnerDatabaseHelper", "Tables created: dinner, calories_summary");
    }


    public void updateCaloriesSummaryDinner(String date) {
        // Выполняем запрос для получения суммы калорий по выбранной дате с округлением до сотых
        String query = "SELECT ROUND(SUM(" + COLUMN_CALORIES + "), 2) FROM " + TABLE_DINNER +
                " WHERE " + COLUMN_DATE + " = ?";
    ...
    public double getTotalCaloriesSummaryDinner(String date) {
    ...
    public void updateProteinSummaryDinner(String date) {
        // Выполняем запрос для получения суммы калорий по выбранной дате с округлением до сотых
        String query = "SELECT ROUND(SUM(" + COLUMN_PROTEIN + "), 2) FROM " + TABLE_DINNER +
                " WHERE " + COLUMN_DATE + " = ?";
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery(query, new String[]{date});
    ...
    public double getTotalProteinSummaryDinner(String date) {
    ...
    public void updateFatSummaryDinner(String date) {
        // Выполняем запрос для получения суммы калорий по выбранной дате с округлением до сотых
        String query = "SELECT ROUND(SUM(" + COLUMN_FAT + "), 2) FROM " + TABLE_DINNER +
                " WHERE " + COLUMN_DATE + " = ?";
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery(query, new String[]{date});
    ...
    public double getTotalFatSummaryDinner(String date) {
        // Выполняем запрос для получения суммы калорий из таблицы protein_summary по выбранной дате
    ...
    public void updateCarbSummaryDinner(String date) {
        // Выполняем запрос для получения суммы калорий по выбранной дате с округлением до сотых
        String query = "SELECT ROUND(SUM(" + COLUMN_CARBOHYDRATE + "), 2) FROM " + TABLE_DINNER +
                " WHERE " + COLUMN_DATE + " = ?";
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery(query, new String[]{date});
    ...
    public double getTotalCarbSummaryDinner(String date) {
        // Выполняем запрос для получения суммы калорий из таблицы protein_summary по выбранной дате
    ...
    public List<Pair<String, Integer>> getTop3CategoriesWithCount() {
        ...
        String query = "SELECT " + COLUMN_CATEGORY + ", COUNT(*) AS count FROM " + TABLE_DINNER +
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        Log.d("DinnerDatabaseHelper", "Upgrading database from version " + oldVersion + " to " + newVersion);
